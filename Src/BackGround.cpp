...
void BackGround::DimensionTransformation(GLfloat source[],GLfloat target[][4])
{
...
void BackGround::Begin(int programIndex)
{
	shaderProgram = shaderPrograms[programIndex];
	shaderProgram->bind();
	vao.bind();
}
void BackGround::Paint(GLfloat* ProjectionMatrix, GLfloat* ModelViewMatrix)
{
	GLfloat P[4][4];
	GLfloat MV[4][4];
...
{
	InitShader("./Shader/background.vs", "./Shader/background.fs");	
	InitVAO();
	InitVBO();
}
...
void BackGround::InitShader(QString vertexShaderPath,QString fragmentShaderPath)
{
	// Create Shader
