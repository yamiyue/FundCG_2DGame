...
void Character::DimensionTransformation(GLfloat source[],GLfloat target[][4])
{
...
void Character::Begin(int programIndex)
{
...
void Character::Paint(GLfloat* ProjectionMatrix, GLfloat* ModelViewMatrix)
{
	GLfloat P[4][4];
	GLfloat MV[4][4];
...
void Character::Init()
{
	InitShader("./Shader/char.vs", "./Shader/char.fs");	
	InitShader("./Shader/char1.vs", "./Shader/char1.fs");	
	InitVAO();
	InitVBO();
}

void Character::InitVAO()
{
	vao.create();
	vao.bind();
}

void Character::InitVBO()
{
...
void Character::InitShader(QString vertexShaderPath,QString fragmentShaderPath)
{
	// Create Shader
