...
void TV::DimensionTransformation(GLfloat source[],GLfloat target[][4])
{
...
void TV::Begin(int programIndex)
{
	shaderProgram = shaderPrograms[programIndex];
	shaderProgram->bind();
	vao.bind();
}
void TV::Paint(GLfloat* ProjectionMatrix, GLfloat* ModelViewMatrix)
{
	GLfloat P[4][4];
	GLfloat MV[4][4];
...
void TV::Init()
{
	InitShader("./Shader/TV.vs", "./Shader/TV.fs");	
	InitVAO();
	InitVBO();
}
void TV::InitVAO()
...
void TV::InitShader(QString vertexShaderPath,QString fragmentShaderPath)
{
	// Create Shader
