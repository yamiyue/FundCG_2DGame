...
void FrameBuffer::DimensionTransformation(GLfloat source[],GLfloat target[][4])
{
...
void FrameBuffer::Begin(int programIndex)
{
	shaderProgram = shaderPrograms[programIndex];
	shaderProgram->bind();
	vao.bind();
}
void FrameBuffer::Paint(GLfloat* ProjectionMatrix, GLfloat* ModelViewMatrix)
{
	GLfloat P[4][4];
	GLfloat MV[4][4];
...
{
	InitShader("./Shader/post_phong.vs", "./Shader/post_phong.fs");	
	InitVAO();
	InitVBO();
}
...
void FrameBuffer::InitShader(QString vertexShaderPath,QString fragmentShaderPath)
{
	// Create Shader
