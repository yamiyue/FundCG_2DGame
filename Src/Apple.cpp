...
void Apple::DimensionTransformation(GLfloat source[],GLfloat target[][4])
{
...
void Apple::Begin(int programIndex)
{
	shaderProgram = shaderPrograms[programIndex];
	shaderProgram->bind();
	vao.bind();
}
void Apple::Paint(GLfloat* ProjectionMatrix, GLfloat* ModelViewMatrix)
{
	GLfloat P[4][4];
	GLfloat MV[4][4];
...
void Apple::Init()
{
	InitShader("./Shader/apple.vs", "./Shader/apple.fs");	
	InitVAO();
	InitVBO();
}
void Apple::InitVAO()
...
void Apple::InitShader(QString vertexShaderPath,QString fragmentShaderPath)
{
	// Create Shader
