...
void main()
{
	...
	vec2 auv = vUV;
	auv *= 1.0-(maxshake*mag);
	
	//	abber
	vec2 texelSize = vec2(0.01, 0.01) / iResolution.xy;
    
	vec2 shakedUV = vec2(auv + shakexy);
	
    float uvXOffset = bx2(shakedUV.x);
    
    float uvXFromCenter = uvXOffset - 150;
    float finalUVX = uvXFromCenter * abs(uvXFromCenter) * aberrationStrength;
    
    float redChannel = texture2D(Texture, vec2(shakedUV.x + (finalUVX * (redShift * texelSize.x)), shakedUV.y)).r;
    float greenChannel = texture2D(Texture, vec2(shakedUV.x + (finalUVX * (greenShift * texelSize.x)), shakedUV.y)).g;
    float blueChannel = texture2D(Texture, vec2(shakedUV.x + (finalUVX * (blueShift * texelSize.x)), shakedUV.y)).b;
	
	if(isFinish)
		textureColor = vec4(redChannel, greenChannel, blueChannel, 1.0);
	else
		textureColor = texture2D(Texture, vUV);
